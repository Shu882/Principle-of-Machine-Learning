...
class Node:
    ...
    A split:
    """
    def __init__(self, feature, left, right, entropy_score, depth=0):
...
def decision_tree_train(data, max_depth=float('inf'), current_depth=None):
    """
    train a decision tree(cf. CIML p13), using entropy for feature selection at every node (cf. ESL p309)
    :param data: andas data frame, training data, labels in the first column and the rest are features
    :param max_depth:
    :param current_depth:
    ...
    elif features.shape[1] == 0 or current_depth == 0:
        return Leaf(guess)
    # base case: max_depth has reached
    else:
        ...
        left = decision_tree_train(no_set, max_depth=max_depth, current_depth=current_depth)
        right = decision_tree_train(yes_set, max_depth=max_depth, current_depth=current_depth)
...
# util
root = decision_tree_train(training_data, max_depth=2)
print_tree(root)
